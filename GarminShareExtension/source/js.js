...
(function addMenuItem() {
    ...
    let waitExist = setInterval(function () {
        if (document.getElementsByClassName("icon-workouts")[0]) {
            ...
            document.getElementsByClassName("icon-workouts")[0].parentElement.parentElement.getElementsByTagName("ul")[0].appendChild(li)
            document.querySelector("a[href*='/modern/workouts']").addEventListener("click", addExportIcon)
...
(function getDevices() {
    ...
    fetch("https://connect.garmin.com/modern/proxy/device-service/deviceregistration/devices", requestConfig)
        .then(response => {
...
function addExportIcon() {
    let waitExist = setInterval(function () {
        if (document.getElementsByClassName("workout-actions")[0]) {
            clearInterval(waitExist)
            for (let i = 0; i < (document.getElementsByClassName("workout-actions").length); i++) {
                let span = document.createElement("span")
                let parent = document.getElementsByClassName("workout-actions")[i]
                span.className = "shareIconSpan"
                span.dataset.id = document.getElementsByClassName("send-to-device")[i].dataset.id
                span.dataset.order = "" + i
                span.addEventListener('click', function () {
                    exportTraining(document.getElementsByClassName("send-to-device")[i].dataset.id)
                })
...
function exportTraining(id) {
    ...
    fetch("https://connect.garmin.com/modern/proxy/workout-service/workout/" + id, requestConfig)
        ...
        }).then(() => notification(exportWorkoutName + chrome.i18n.getMessage("clipboard")))
}
...
function showDialog() {
    ...
    compatibleHeadText.innerText = devicesInfo.length === 1 ? chrome.i18n.getMessage("compatibleDevice"):chrome.i18n.getMessage("compatibleDevices")
    ...
    let clipboardData = (navigator.clipboard.readText()).then(response => JSON.parse(response)).then(result => {
        if (result.workoutName) {
            bodyInput.style.display = "none"
            bodyText.innerText = chrome.i18n.getMessage("bodyText") + result.workoutName + "?"
            trainingData = result
...
function importTraining() {
    ...
    let requestConfig = {
        ...
        "referrer": "https://connect.garmin.com/modern/workout/create/strength_training",
        "referrerPolicy": "strict-origin-when-cross-origin",
    ...
    fetch("https://connect.garmin.com/modern/proxy/workout-service/workout", requestConfig)
        .then(response => response.json())
        .then(result => {
            workoutId = result.workoutId;
            importWorkoutName = result.workoutName
        })
        .then(() => (document.getElementById("onlyImport").checked) ? showNotification(true) : sendTraining())
}

function checkImportProperties() {
    targetDevices = []
    if (!trainingData && !((document.getElementById("bodyInput").value).includes("workoutName"))) {
        alert("data")
    } else {
        if (document.getElementById("onlyImport").checked === false) {
            for (let i = 0; i < document.getElementsByName("deviceCheckbox").length; i++) {
...
function sendTraining() {
    for (let i = 0; i < targetDevices.length; i++) {
        ...
        fetch("https://connect.garmin.com/modern/proxy/device-service/devicemessage/messages", requestConfig)
    }
...
function showNotification(onlyImport) {
    ...
    for (let i = 0; i < targetDevices.length; i++) {
        ...
        devices += (i === 0 ? " " : (i === targetDevices.length - 1 ? " & " : ", ")) + name
    }
    notification(trainingData.workoutName + chrome.i18n.getMessage("imported") + (onlyImport ? "" : chrome.i18n.getMessage("andSent") + devices) + ".")
    closeDialog()
}
